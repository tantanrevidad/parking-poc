...
# Inject popover theme via components.html (only way to run JS in Streamlit).
# The iframe created by components.html has same-origin access to window.parent.
# Strategy: inject a <style> tag into parent document.head AND run a MutationObserver.
...
try {{
    ...
    // --- 1. Inject CSS into parent <head> for body-level popovers ---
    var existingStyle = doc.getElementById('popover-theme-override');
    if (existingStyle) existingStyle.remove();

    var style = doc.createElement('style');
    style.id = 'popover-theme-override';
    style.textContent = `
        [data-baseweb="popover"],
        [data-baseweb="popover"] > div,
        [data-baseweb="popover"] > div > div,
        [data-baseweb="popover"] > div > div > div,
        [data-baseweb="popover"] > div > div > div > div,
        [data-baseweb="menu"],
        ...
        li[role="option"] span,
        li[role="option"] p {{
            background-color: { "#111827" if st.session_state.theme_mode == "dark" else "#FFFFFF" } !important;
            background: { "#111827" if st.session_state.theme_mode == "dark" else "#FFFFFF" } !important;
            color: { "#F8FAFC" if st.session_state.theme_mode == "dark" else "#0F172A" } !important;
            -webkit-text-fill-color: { "#F8FAFC" if st.session_state.theme_mode == "dark" else "#0F172A" } !important;
            border-color: { "#1F2937" if st.session_state.theme_mode == "dark" else "#CBD5E1" } !important;
        }}
        ...
        li[role="option"][aria-selected="true"] * {{
            background-color: { "#1E293B" if st.session_state.theme_mode == "dark" else "#F1F5F9" } !important;
            background: { "#1E293B" if st.session_state.theme_mode == "dark" else "#F1F5F9" } !important;
            color: { "#38BDF8" if st.session_state.theme_mode == "dark" else "#0284C7" } !important;
            -webkit-text-fill-color: { "#38BDF8" if st.session_state.theme_mode == "dark" else "#0284C7" } !important;
        }}
    `;
    doc.head.appendChild(style);

    // --- 2. MutationObserver for inline style overrides ---
    var isDark = {_is_dark_js};
    var bg = isDark ? '#111827' : '#FFFFFF';
    var bgHover = isDark ? '#1E293B' : '#F1F5F9';
    var textColor = isDark ? '#F8FAFC' : '#0F172A';
    var hoverColor = isDark ? '#38BDF8' : '#0284C7';
    var borderColor = isDark ? '#1F2937' : '#CBD5E1';
    ...
    function fixPopover(p) {{
        fix(p);
        p.style.setProperty('border', '1px solid ' + borderColor, 'important');
        p.style.setProperty('border-radius', '10px', 'important');
        p.querySelectorAll('*').forEach(fix);
    }}

    if (window.__popoverObs) window.__popoverObs.disconnect();
    var obs = new MutationObserver(function(muts) {{
        for (var i = 0; i < muts.length; i++) {{
            for (var j = 0; j < muts[i].addedNodes.length; j++) {{
                var n = muts[i].addedNodes[j];
                if (n.nodeType !== 1) continue;
                if (n.getAttribute && n.getAttribute('data-baseweb') === 'popover') {{
                    setTimeout(function(el){{ fixPopover(el); }}, 20, n);
                }}
                if (n.querySelectorAll) {{
                    var ps = n.querySelectorAll('[data-baseweb="popover"]');
                    for (var k = 0; k < ps.length; k++) {{
                        setTimeout(function(el){{ fixPopover(el); }}, 20, ps[k]);
                    }}
                }}
            }}
        }}
    ...
    window.__popoverObs = obs;
}} catch(e) {{
    // silently fail if cross-origin
}}
</script>
"""
components.html(_popover_fix, height=0, width=0)
